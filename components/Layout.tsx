...
import { 
  ...
  Menu, X, Calendar, Star, Sparkles, ChevronDown
} from 'lucide-react';
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-brand-main text-brand-text font-sans flex flex-col selection:bg-brand-gold selection:text-brand-main overflow-x-hidden">
      {/* Header - Adaptive for all screens */}
        ...
        <div className="container mx-auto px-4">
          <div className={`mx-auto max-w-7xl px-4 md:px-8 py-2 md:py-3 rounded-full border transition-all duration-500 flex items-center justify-between gap-2 md:gap-4 ${
            ...
            <Link to="/" className="flex items-center gap-2 group shrink-0">
              <div className="bg-brand-gold p-1.5 md:p-2 rounded-xl shadow-glow group-hover:rotate-12 transition-all">
            ...
            <div className="hidden lg:flex items-center flex-1 max-w-xs mx-4">
               <div className="relative w-full">
                  ...
                  <input type="text" placeholder={t.search} className="w-full bg-white/5 border border-white/10 rounded-full py-2 pr-10 pl-4 text-xs text-white outline-none focus:border-brand-gold/50" />
               </div>
            </div>
            ...
            <nav className="hidden md:flex items-center gap-1 bg-white/5 p-1 rounded-full border border-white/5">
                ...
                {isAdmin && <Link to="/admin" className="px-4 py-1.5 rounded-full text-xs font-black text-orange-500 bg-orange-500/10 border border-orange-500/20 ml-2">ADMIN</Link>}
            </nav>

            <div className="flex items-center gap-2">
               <button onClick={toggleLanguage} className="p-2 rounded-full bg-white/5 text-brand-muted hover:text-brand-gold transition-all">
                 <Languages size={18} />
               </button>
               ...
               {user ? (
                 <div className="relative flex items-center gap-2" ref={dropdownRef}>
                    ...
                    {isProfileOpen && (
                      <div className="absolute top-full mt-4 left-0 md:left-auto md:right-0 w-64 bg-brand-card/95 backdrop-blur-3xl border border-white/10 rounded-3xl shadow-2xl p-3 animate-scale-up z-[200]">
                        <div className="p-4 border-b border-white/5 mb-2">
                           <p className="text-white font-black text-sm truncate">{user.name}</p>
                           <p className="text-[10px] text-brand-gold font-bold uppercase tracking-widest">{user.subscriptionTier} student</p>
                        </div>
                 ...
                 <Link to="/login" className="px-5 py-2 rounded-full bg-brand-gold text-brand-main font-black text-xs shadow-glow">دخول</Link>
               )}
            </div>
          </div>
        </div>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-[150] w-[90%] max-w-sm">
         <div className="bg-brand-card/90 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-2 flex items-center justify-around">
            ...
            <Link to="/wallet" className={`flex flex-col items-center gap-1 p-3 rounded-2xl transition-all ${isActive('/wallet') ? 'text-brand-gold bg-brand-gold/10' : 'text-brand-muted'}`}>
               <Wallet size={20} />
               <span className="text-[8px] font-black uppercase">{t.wallet}</span>
            </Link>
